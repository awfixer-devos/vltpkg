...
describe('fetchRegistryManifest', () => {
  ...
  test('uses latest for version ranges', async () => {
    // When bareSpec is a range like ^1.0.0, should request /latest instead
    const spec = Spec.parse('my-package', '^1.0.0')
    const result = await runWithMock(
      fetchRegistryManifest({ spec: spec.final }),
      {
        'registry.npmjs.org/my-package/latest': {
          name: 'my-package',
          version: '1.5.0',
        },
      },
  ...
  test('uses latest for tilde ranges', async () => {
    const spec = Spec.parse('my-package', '~2.0.0')
    const result = await runWithMock(
      fetchRegistryManifest({ spec: spec.final }),
      {
        'registry.npmjs.org/my-package/latest': {
  ...
  test('uses latest for wildcard', async () => {
    const spec = Spec.parse('my-package', '*')
    const result = await runWithMock(
      fetchRegistryManifest({ spec: spec.final }),
      {
        'registry.npmjs.org/my-package/latest': {
